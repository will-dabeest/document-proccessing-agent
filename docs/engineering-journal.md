...
- 2026-05-13: Compose-first local dev — extended `infra/docker-compose.yml` (LocalStack healthcheck, Terraform init/apply job, ingestion/worker/frontend builds via `infra/Dockerfile.*`, optional Ollama behind profile `llm` — superseded 2026-05-14); `.env.example`, `docs/local-dev.md`, CI job `compose-config`.
- 2026-05-14: Compose always starts **ollama** (removed `llm` profile); ingestion/worker get `OLLAMA_BASE_URL=http://ollama:11434` from compose; docs and architecture diagrams updated.

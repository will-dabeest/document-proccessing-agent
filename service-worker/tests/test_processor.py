...
def test_process_job_body_none_classification_and_summary_are_persisted():
    """Explicit None bypasses .get defaults, unlike a missing key (Unknown / "")."""
    tbl = MagicMock()
    with patch(
        "worker_app.processor.download_object_bytes", return_value=b"bytes"
    ), patch(
        ...
        "worker_app.processor.run_agent",
        return_value={"classification": None, "summary": None},
    ), patch(
        "worker_app.processor.save_completed"
    ) as sc, patch(
        "worker_app.processor.notify_index"
    ):
        process_job_body("job-none-cls", "file.txt", tbl)

    sc.assert_called_once_with(tbl, "job-none-cls", None, None)

...
def test_notify_index_posts_internal_index_payload():
    with patch(
        "worker_app.processor.settings.ingestion_base_url", "http://ingestion.test/"
    ), patch("httpx.post") as post:
        notify_index("job-1", "doc.pdf", "extracted text")

    post.assert_called_once_with(
        "http://ingestion.test/internal/index",
        json={"job_id": "job-1", "filename": "doc.pdf", "text": "extracted text"},
        timeout=30.0,
    )


def test_notify_index_swallows_post_failure(caplog):
    with patch("httpx.post", side_effect=RuntimeError("ingestion down")) as post:
        notify_index("job-2", "doc.pdf", "extracted text")

    post.assert_called_once()
    assert "index_notify_failed" in caplog.text
